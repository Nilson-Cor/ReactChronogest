...
import { Calendar, Clock, MapPin, BookOpen, Users, RefreshCw, GraduationCap } from 'lucide-react';

const API_URL = 'http://localhost:5000'; // Ajusta según tu configuración

    interface HorarioFicha {
    id: string;
    dia_semana: string;
    hora_inicio: string;
    hora_fin: string;
    ...
    const HorarioAprendiz: React.FC<Props> = ({ fichaId, fichaNumero, programaNombre }) => {
    const [horariosAsignados, setHorariosAsignados] = useState<HorarioFicha[]>([]);
    ...
    const loadHorariosFicha = async () => {
        setLoading(true);
        try {
        const response = await fetch(`${API_URL}/api/horarios/ficha/${fichaId}`, {
        ...
        console.error('Error cargando horarios de la ficha:', error);
    ...
    const agruparPorDia = () => {
        const agrupado: { [key: string]: HorarioFicha[] } = {};
    ...
    const instructoresUnicos = new Set(
        horariosAsignados
        .map(item => item.instructor_nombre)
        .filter(Boolean)
    );
    ...
    return (
        ...
        <div className="flex items-center justify-between mb-6">
            ...
            <div>
                ...
                <p className="text-sm text-gray-600">Horario de clases semanal</p>
            </div>
            </div>
            <button
            onClick={loadHorariosFicha}
        ...
        <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
            ...
            Información de tu Ficha
            </h3>
            <div className="grid grid-cols-3 gap-4 text-sm">
            <div>
                <span className="text-gray-600 block mb-1">Ficha:</span>
                <p className="font-bold text-gray-800">{fichaNumero}</p>
            </div>
            <div>
                <span className="text-gray-600 block mb-1">Programa:</span>
                <p className="font-bold text-gray-800">{programaNombre}</p>
            </div>
            <div>
                <span className="text-gray-600 block mb-1">Centro:</span>
                <p className="font-bold text-gray-800">Centro de Gestión y Desarrollo Surcolombiano</p>
            </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
            ...
            <div className="text-center">
                <div className="text-3xl font-bold text-orange-600">{instructoresUnicos.size}</div>
        ...
        {loading ? (
            ...
            <p className="text-sm text-gray-500">El administrador aún no ha asignado horarios a tu ficha</p>
            </div>
        ) : (
            <div>
            ...
            {instructoresUnicos.size > 0 && (
                ...
                <div className="grid grid-cols-3 gap-3">
                    {Array.from(instructoresUnicos).map((instructor, index) => (
                    <div key={index} className="bg-white px-4 py-3 rounded-lg text-gray-800 font-medium text-center">
                        {instructor}
                    </div>
                    ))}
                </div>
...
export default HorarioAprendiz;